...
1. **Integrated Data is Fragile:** Dense image networks (MNIST) suffered catastrophic forgetting under heavy-tailed noise. Across all defense strategies, injecting Laplace noise plummeted the image model's accuracy to unusable levels (dropping from >90% to below 50%).
2. **Scattered Data is Robust:** The tabular networks (Breast Cancer features) showed incredible resilience. They comfortably absorbed aggressive Laplace noise, maintaining exceptional >97% accuracy in every single experiment.

**Conclusion:** Robust aggregation (Trimmed Mean, Clipping, FedProx) protects against data poisoning and client drift, but statistical privacy mechanisms must be uniquely tailored to the data's underlying structure.
